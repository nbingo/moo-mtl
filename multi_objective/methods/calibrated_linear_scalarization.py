...
class CalibratedLinearScalarizationMethod(BaseMethod):
    ...
    def _step(self, batch, model, preference_ray):
        ...
        for i, objective in enumerate(self.objectives.values()):
            ...
            loss_total += task_loss * preference_ray[i]
        cossim = torch.nn.functional.cosine_similarity(torch.stack(task_losses), torch.from_numpy(preference_ray).to(self.device), dim=0)
